                    ...
                    <div class="card card-user">
                        <div class="card-header">
                            <div class="mt-2 ml-2 me-3">
                                <a href="/tassky/view/list/Roles-list.php" class="text-decoration-none btn btn-outline-primary btn-round">Back</a>
                            </div>
                            <div>
                                <?php
                                if (isset($_GET['id'])) {
                                ...
                                ?>
                                <h5 class="card-title text-center">Assign Permission to Role </h5>
                            </div>
                        </div>
                        <div class="card-body">
                                ...
                                <div class="row px-3">
                                    ...
                                    <div class="d-flex flex-column justify-content-center align-items-start pb-3 px-3">
                                        ...
                                        foreach ($moduleController as $moduleControllers) {
                                            $moduleName = $moduleControllers['name'];
                                        ?>
                                            <!-- ======================================== -->
                                            <!-- show all module -->
                                            <div class="d-flex flex-column justify-content-start align-items-start">
                                                <div class="mb-1">
                                                    <h4 style="text-transform: capitalize;"><?php echo $moduleName; ?></h4>
                                                    <?php
                                                    $actions = new Actions();
                                                    ...
                                                    <div class="d-flex justify-content-center align-items-center flex-wrap px-3">
                                                        ...
                                                        <?php if (is_array($actionController) && count($actionController) > 0) : ?>
                                                            <?php foreach ($actionController as $actionControllers) : ?>
                                                                <?php

                                                                if (Sentinel::getUser()) {
                                                                    $roles = Sentinel::findRoleById($roleId);
                                                                    if ($roles !== false) {
                                                                        $desiredPermission = $moduleControllers['slug'] . "." . $actionControllers['slug'];
                                                                        $isChecked = false;
                                                                        if (is_array($roles)) {
                                                                            foreach ($roles as $role) {
                                                                                if ($role->hasAccess($desiredPermission)) {
                                                                                    $isChecked = true;
                                                                                    break;
                                                                                }
                                                                            }
                                                                        } else {
                                                                            if ($roles->hasAccess($desiredPermission)) {
                                                                                $isChecked = true;
                                                                            }
                                                                        }
                                                                ?>
                                                                        <div class="d-flex justify-content-center align-items-center py-2 px-2">
                                                                            <label style="text-transform: capitalize;" for="" class="mx-2"><?php echo $actionControllers['name'] ?></label>
                                                                            <input class="py-1" type="checkbox" name="actions[]" value="<?php echo $desiredPermission ?>" <?php echo $isChecked ? 'checked' : '' ?> class="mx-2">
                                                                        </div>
                                                                <?php } else {
                                                                        echo "<p>Error: Role not found.</p>";
                                                                    }
                                                                }
                                                                ?>
                                                            <?php endforeach; ?>
